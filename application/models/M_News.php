...
class M_News extends CI_Model {
    ...
    public function getSport_lastest_news($sport_id) {
        return $this->db->query("SELECT news.*, user.fullname
        FROM `news`, user where news.sport_type = $sport_id and news.user_id = user.id and news.news_status = 'published' order by news.created_at DESC;")->row();
    }

    public function getSport_lastest_news_result($sport_id) {
        return $this->db->query("SELECT news.*, user.fullname
        FROM `news`, user where news.sport_type = $sport_id and news.user_id = user.id and news.news_status = 'published' ORDER BY created_at DESC LIMIT 5;")->result();
    }
    ...
    public function getNews_by_sport($sport_id) {
        return $this->db->query("SELECT news.*, user.fullname 
                                 FROM `news`, user where news.user_id = user.id and news.news_status = 'published' and news.sport_type ='$sport_id' ")->result();
    }
